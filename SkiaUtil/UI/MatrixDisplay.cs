...
namespace SkiaUtil.UI
{
    class MatrixDisplay
    {
        ...
        {
            //var sizeUtil = DependencyService.Get<IScreenSize>();

            float xdpi = 200f;
            float ydpi = 200f;
            var defPaint = new SKPaint()
            {
                Color = SKColors.Black,
                TextSize =    (float)(2.5 * ydpi / 25.4),
                StrokeWidth = (float)(0.2 * xdpi / 25.4)
            };

            MatrixPaint = defPaint;
        }

        public SKPaint MatrixPaint { set; get; }
